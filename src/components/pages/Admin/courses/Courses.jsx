...
import CourseActions from "@/components/dashboard/course-actions-admin"
...
export default function CoursesPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const coursesResponse = await axios.get('http://localhost:8000/api/admin/courses', {
          headers: getAuthHeader()
        })
  ...
  const filteredCourses = courses.filter((course) => {
    ...
    const matchesCategory = categoryFilter ? course.category.name === categoryFilter : true
                ...
                <tbody className="bg-white divide-y divide-gray-200">
                  {filteredCourses.map((course) => (
                    <tr key={course.id}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap">{course.enrollments_count}</td>
