...
import { 
  ArrowLeft, Trash, Calendar, UserRound, 
  Tag, Clock
} from "lucide-react";
...
export default function CourseDetailsPage() {
  ...
  const handleDelete = async () => {
    try {
      ...
      await toast.promise(
        axios.delete(`http://localhost:8000/api/admin/courses/${id}`, {
          headers: getAuthHeader()
        }),
        {
  ...
  const handleStatusUpdate = async () => {
    if (selectedStatus === course.status) {
      toast.error("Le statut n'a pas changé");
      return;
    }
    ...
    try {
      ...
      await toast.promise(
        axios.patch(
          `http://localhost:8000/api/admin/courses/${id}/status`,
          { status: selectedStatus },
          { headers: getAuthHeader() }
        ),
        {
          loading: 'Mise à jour du statut...',
          success: selectedStatus === 'active'
